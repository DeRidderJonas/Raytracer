...
#pragma region ArithmeticOperators
		...
		template<typename U>
		inline Point<3, T> operator+(const Point<3, U>& p) const
		{ return Point<3, T>(x + static_cast<T>(p.x), y + static_cast<T>(p.y), z + static_cast<T>(p.z)); }
		...
		template<typename U>
		inline Point<3, T> operator/(U scale) const
